   ...
   window.openEntityModal = async(id = null) => {
    ...
    function getMaxLenDoc() {
        if (docTypeSelect.value === 'DNI') return 8;
        if (docTypeSelect.value === 'RUC') return 11;
        return 11; 
    }

    function sanitizeDocInput() {
        const maxLen = getMaxLenDoc();
        docInput.value = docInput.value.replace(/\D/g, '').slice(0, maxLen);
    }

    docInput.setAttribute('maxlength', getMaxLenDoc());
    ...
    document.getElementById('ent-form').addEventListener('submit', async ev => {
        ...
        if (guardadoExitoso) {
            ...
            Swal.fire({
                title: 'Error',
                text: e.message || 'No se pudo guardar la entidad en el servidor.',
                icon: 'error',
                customClass: { popup: 'rounded-2xl' }
            });
        }
